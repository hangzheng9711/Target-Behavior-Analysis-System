...
if ($_POST['json'])//接受json字符串
{
	...
	$abnormal_time=json_encode($abnormal_time);
	$total_time=json_encode($total_time);
	$result = json_encode(array("station_count" => $station_count,"abnormal_time" => $abnormal_time,"total_time"=> $total_time)); //将php数组封装为json字符串
