...
if ($_POST['json'])//接受json字符串
{
	...
	$sstartpoint=$jsonstr['startpoint']-1;
	$list_len=$jsonstr['list_len'];
	
	$year=$jsonstr['year'];
	$start = mktime(0,0,0,1,1,$year);
	$year=$year+1;
	$end = mktime(0,0,0,1,1,$year);
	$sql="SELECT * FROM behavior WHERE UNIX_TIMESTAMP(start_time)>=$start and UNIX_TIMESTAMP(start_time)<$end ORDER BY start_time limit $sstartpoint,$list_len";
